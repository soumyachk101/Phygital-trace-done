...
import { TruthAttestation } from "../typechain-types";
import { SignerWithAddress } from "@nomicfoundation/hardhat-ethers/signers";
import { HardhatEthersSigner } from "@nomicfoundation/hardhat-ethers/signers";

describe("TruthAttestation", function () {
  let attestation: TruthAttestation;
  let owner: HardhatEthersSigner;
  let addr1: HardhatEthersSigner;
  let addr2: HardhatEthersSigner;

  const payloadHash = ethers.id("test-payload-1");
  const ipfsCidBytes32 = ethers.formatBytes32String("QmTest123");
  ...
  describe("attest()", () => {
    ...
    it("emits Attested event", async () => {
      await expect(attestation.attest(payloadHash, ipfsCidBytes32))
        ...
        .withArgs(payloadHash, "QmTest123", await attestation.verify(payloadHash).then(r => r.timestamp), owner.address);
    });
  });
  ...
  describe("attestBatch()", () => {
    it("attests multiple payloads", async () => {
      ...
      const cids = hashes.map((_, i) => ethers.formatBytes32String(`cid${i}`));
    ...
    it("rejects length mismatch", async () => {
      await expect(
        attestation.attestBatch(
          [ethers.id("p1")],
          [ethers.formatBytes32String("c1"), ethers.formatBytes32String("c2")]
        )
    ...
    it("skips already attested in batch", async () => {
      ...
      const c1 = ethers.formatBytes32String("c1");
      await attestation.attest(h1, c1);
      const h2 = ethers.id("p2");
      const c2 = ethers.formatBytes32String("c2");
  ...
  describe("revoke()", () => {
    ...
    it("rejects non-owner revoke", async () => {
      await attestation.attest(payloadHash, ipfsCidBytes32);
      await expect(attestation.connect(addr1).revoke(payloadHash)).to.be.reverted;
    });
    ...
    it("emits Revoked event", async () => {
      ...
      await expect(attestation.revoke(payloadHash))
        ...
        .withArgs(payloadHash, owner.address, await ethers.provider.getBlock("latest").then(b => b!.timestamp));
    });
  });
  ...
  describe("pause/unpause", () => {
    ...
    it("rejects non-owner pause", async () => {
      await expect(attestation.connect(addr1).pause()).to.be.reverted;
    });
  });
});
